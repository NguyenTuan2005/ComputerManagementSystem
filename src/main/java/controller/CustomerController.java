...
public class CustomerController implements ModelController<Customer> {
  ...
  public void saves(ArrayList<Customer> customers) {
    for (Customer customer : customers) customerDAO.save(customer);
  }
  ...
  public ArrayList<CustomerOrderDetailDTO> getCustomerOrderDetail(int id) {
    ArrayList<CustomerOrderDTO> customerOrderDetailDTOs =
        this.customerDAO.getDataCustomerOrderById(id);
    ...
    for (var data : customerOrderDetailDTOs) {
      result.add(
          new CustomerOrderDetailDTO(
              data,
              images.stream()
                  .filter(image -> image.getProductId() == data.getProductId())
