...
public class AccountDAO implements Repository<Account> {
  ...
  private static void setAccountParameter(PreparedStatement preparedStatement, Account account)
      throws SQLException {
  ...
  public Account save(Account account) {
    if (account.getId() == 0) {
      // Insert new account
      String sql =
          "INSERT INTO account (username, password, email, create_date, manage_id , avata_img) VALUES (?, ?, ?, ?, ?,?)";
      try {
  ...
  public static void main(String[] args) throws SQLException {
    ...
    accountDAO.updateById(
        new Account(
            12, "thanh update", "root", "duvg@gmail.com", new Date(9, 9, 2004), 1, "hehehe"));
  }
  ...
  public Account updateById(Account account) {
    String sql =
        "UPDATE account SET username = ? , email =? ,create_date =?,avata_img =? WHERE  id = ? ";
    try {
      PreparedStatement preparedStatement = connection.prepareStatement(sql);
      preparedStatement.setString(1, account.getUsername());
