...
public class ManagerDAO implements Repository<Manager> {
  ...
  public Manager save(Manager manager) {
    String sql =
        "INSERT INTO manager (fullname, address, birthday, phone_number) VALUES (?, ?, ?, ?)";
    try {
  ...
  public Manager update(Manager manager) {
    String sql =
        "UPDATE Manager SET fullname = ?, address = ?, birthday = ?, phone_number = ? WHERE id = ?";
    try {
      preparedStatement = connection.prepareStatement(sql);
      preparedStatement.setString(1, manager.getFullName());
      preparedStatement.setString(2, manager.getAddress());
  ...
  public ManagerInforDTO getManagerWithAccountByName(String nameOfAccount) {
    String sql =
        "SELECT m.id AS managerId, m.fullname, m.address, m.birthday, m.phone_number, "
