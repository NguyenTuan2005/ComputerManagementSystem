...
public class OrderDAO implements Repository<Order> {
  ...
  public Order save(Order order) {
    String query =
        "INSERT INTO public.order (manager_id, customer_id, order_date, ship_address, status) VALUES (?, ?, ?, ?, ?)";
    try {
  ...
  public ArrayList<Order> findByName(String name) {
    ArrayList<Order> orders = new ArrayList<>();
    String query =
        "SELECT * FROM public.order WHERE customer_id IN (SELECT id FROM public.customers WHERE name LIKE ?)";
    try {
      preparedStatement = connection.prepareStatement(query);
  ...
  public Order findOneByName(String name) {
    Order order = null;
    String query =
        "SELECT * FROM public.order WHERE customer_id IN (SELECT id FROM public.customers WHERE name = ?)";
    try {
      preparedStatement = connection.prepareStatement(query);
      preparedStatement.setString(1, name);
      ResultSet rs = preparedStatement.executeQuery();
  ...
  public boolean updateOrderStatus(OrderType type, int id) {
    ...
    try {
      ...
      ToastNotification.showToast(
          "The quantity of goods in stock is sufficient", 30000000, 30, -1, -1);
      e.printStackTrace();
    }
    return false;
  }
  ...
  public Order update(Order order) {
    String query =
        "UPDATE public.order SET manager_id = ?, customer_id = ?, order_date = ?, ship_address = ?, status = ? WHERE id = ?";
    try {
      preparedStatement = connection.prepareStatement(query);
      preparedStatement.setInt(1, order.getManagerId());
      preparedStatement.setInt(2, order.getCustomerId());
