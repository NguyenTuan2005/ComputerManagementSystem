...
public class SupplierDAO implements Repository<Supplier> {
  ...
  public Supplier save(Supplier supplier) {
    String query =
        "INSERT INTO public.suppliers (company_name, email, phone_number, address, contract_date) VALUES (?, ?, ?, ?, ?)";
    try {
  ...
  public Supplier update(Supplier supplier) {
    ...
    String query =
        "UPDATE public.suppliers SET company_name = ?, email = ?, phone_number = ?, address = ?, contract_date = ? WHERE id = ?";
    try {
      preparedStatement = connection.prepareStatement(query);
      preparedStatement.setString(1, supplier.getCompanyName());
      preparedStatement.setString(2, supplier.getEmail());
  ...
  public static void main(String[] args) {
    ...
    for (Supplier supplier : supplierDAO.getAll()) System.out.println(supplier);
