...
public class DataOfImage {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < imageUrls.length; i++) {
            ...
            String fileName = imageUrls[i].substring(imageUrls[i].lastIndexOf('/') + 1, imageUrls[i].lastIndexOf('.'));

            // Thay thế dấu gạch dưới bằng khoảng trắng
            laptopNames[i] = fileName.replace('_', ' ');
            // Tạo alt text dài hơn
            altTexts[i] = "This is an image of the " + laptopNames[i] +
                    ". It is a high-performance laptop designed for users who need a powerful and reliable device. " +
                    "The " + laptopNames[i] + " features cutting-edge technology, including fast processors, ample memory, and a sleek design. " +
