...
public class ShowOrder extends JPanel {
   private  int orderId;
    private ArrayList<CustomerOrderDetailDTO> customerOrderDTOs;


    public ShowOrder(Map<Integer , List<CustomerOrderDetailDTO>> map){
        this.setLayout(new BoxLayout(this,BoxLayout.Y_AXIS));
        for (Map.Entry<Integer, List<CustomerOrderDetailDTO>> data : map.entrySet()) {
            System.out.println(">>>>>>>>>>>>>");
           add(showOrder(data.getKey(), (ArrayList<CustomerOrderDetailDTO>) data.getValue()));
        }

    }

    public JPanel  showOrder(int orderId , ArrayList<CustomerOrderDetailDTO> customerOrderDTOS){
        ...
        JLabel totalPriceLabel = LabelConfig.createLabel(priceFormated + "₫", Style.FONT_BOLD_25, Style.CONFIRM_BUTTON_COLOR_GREEN, SwingConstants.RIGHT);
